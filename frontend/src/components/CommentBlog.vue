...
export default {
  ...
  async created() {
    ...
    return fetch(`https://blogi-36jo.onrender.comcomment/${this.blogid}`, {
      method: "GET",
      mode: "cors",
      headers: {
  ...
  methods: {
    async comment() {
      const res = await fetch(
        `https://blogi-36jo.onrender.com/comment/${this.blogid}`,
        {
          method: "POST",
          mode: "cors",
          headers: {
    ...
    async del(id) {
      ...
      if (confirm("Want to delete this comment !!")) {
        const res = await fetch(`https://blogi-36jo.onrender.com/api/comment/${id}`, {
          method: "DELETE",
          mode: "cors",
          headers: {
            "Content-Type": "application/json",
